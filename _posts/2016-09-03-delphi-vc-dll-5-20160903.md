...
categories: delphi之dll c/c++之dll c/c++之指针与内存 delphi之指针与内存 delphi之字符串 c/c++之字符串
...
不光是传递字符串，还有其他各种数据类型：整型数组、结构体等，不过因为字符串比较特殊，所以专门拿出来说一下。其实最本质的就是如何正确的使用和管理内存空间！

其实整个流程走完之后，发现这个和是不是因为Delphi配合VC++开发有关，本质原因就是一个内存管理的问题！
...
无法获取Delphi端Char数组的长度，strlen(resp)获取的值是0，因为strlen是通过判断字符串的结束位'\0'来获取字符串的长度的，而上面的Delphi函数中没有对resp进行赋值
...
若CallCpp中有写操作，但是参数resp却没有申请对应合法的内存空间，那么必然就会报错了，以上的各种情况只是表面上的各种形式而已，最本质的是有没有合法地处理内存空间
...
不只是针对Delphi调用C++导出的方法，Delphi自己定义方法时也是这样的，参考文章[《理解Delphi编程中方法参数定义的关键字const》](http://www.xumenger.com/delphi-z-string-const-20160511/)