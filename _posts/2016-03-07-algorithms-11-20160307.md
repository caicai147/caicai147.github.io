...
tags: 算法 数据结构 c c++ 指针 内存 图 邻接表链表
---

##简介

我们主要通过邻接表链表结构来表示图。链表中的每个结构体都包含两个成员：一个顶点以及该顶点向邻接的一个顶点集合
...
int graph_is_adjlist(const Graph &graph, const void *data1, const void *data2);
...
{
    AdgList *adjlist;
    while(list_size(&graph->adglists) > 0)
    {
        if(list_rem_next(&graph->adglists, NULL, (void **)&adjlist) == 0)
        {
            set_destroy(&adjlist->adjacent);
            
            if(graph->destroy != NULL)
                graph->destroy(adjlists->vertex);
...
int graph_ins_edge(Graph *graph, const void *data1, const void *data2)
{
    ...
    for(element = list_head(&grap->adjlists); element != NULL; element = list_next(element))
    {
...
int graph_rem_vertex(Graph *graph, void **data)
{
    ...
    for(element = list_head(&graph->adjlists); element != NULL; element = list_next(element))
    {
        if(set_is_member(&((AdjList *)list_data(eleemnt))->adjacent, *data))
            return -1;
        if(graph->match(*data, ((AdjList *)list_data(element))->vertex))
        {
            temp = element;
